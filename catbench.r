...
if(Sys.info()[['sysname']] == 'Windows') {
	memory.limit(8192)
	dataset_connection <- dbConnect(SQLite(), "D:\\maestriadm\\dm economia finanzas\\bankchurn\\dias\\dataset.sqlite", flags = SQLITE_RWC)
	train_dir <- "D:\\maestriadm\\dm economia finanzas\\bankchurn\\train_dir"
} else {
	dataset_connection <- dbConnect(SQLite(), "/home/rcarlomagno/data/dataset.sqlite", flags = SQLITE_RWC)
	train_dir <- "/home/rcarlomagno/train_dir"
}
...
data_train = dbGetQuery(dataset_connection, paste0('SELECT * FROM periodos WHERE foto_mes IN (', paste(train_periods, collapse = ', '), ')'))
...
data_test = dbGetQuery(dataset_connection, paste0('SELECT * FROM periodos WHERE foto_mes IN (', paste(test_periods, collapse = ', '), ')'))
...
difftime(end_time, start_time, units = "secs") #as.numeric